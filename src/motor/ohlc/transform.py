...
def calculate_log_price(P_t: float) -> float:
    ...
    """
    if P_t <= 0:
        raise ValueError(f"P_t <= 0 for log calculation")
    return math.log(P_t)
...
def transform_to_returns(series) -> TransformResult:
    ...
    Args:
        series: Série temporal de barras validadas (ValidatedSeries)
...
def transform_to_returns_dict(series) -> dict:
