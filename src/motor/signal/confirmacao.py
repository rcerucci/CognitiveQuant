...
def calculate_confirmacao(
    ...
    # Verificar High=Low primeiro (8.6 edge case)
    if len(bar) >= 6:
        high = bar[2]
        low = bar[3]
        if high == low:
    ...
    # 8.6: Skewness extrema - abort → NEUTRO
    ...
    if lb_pvalue is None or lb_pvalue >= 0.05:
        lb_str = f"{lb_pvalue:.4f}" if lb_pvalue is not None else "None"
        return ConfirmacaoResult(
            status=ConfirmacaoStatus.NEUTRO,
            direction=None,
            metrics=metrics,
            reason=f"Ljung-Box p-value >= 0.05: {lb_str} (8.2)",
    ...
    # Todas as confirmações passaram
    return ConfirmacaoResult(
